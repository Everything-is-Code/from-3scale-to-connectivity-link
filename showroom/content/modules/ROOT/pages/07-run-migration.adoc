...
* Run the *Migrate from 3scale to Connectivity Link* Software Template in Developer Hub
...
== Other migration paths (same target)

This module uses the *Software Template* path (scaffolder → Gitea → Argo CD). The lab also ships two GUIs that produce the same Connectivity Link resource types:
...
After policies exist, operate keys and traffic with xref:11-kuadrant-console.adoc[Kuadrant Console].
