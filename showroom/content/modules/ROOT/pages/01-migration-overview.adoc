...
| Software Template → Git → Argo CD → Kuadrant. Self-service GitOps; primary hands-on path in this workshop (xref:07-run-migration.adoc[module 7]).

| *Kuadrant Console*
| Operate API Products, keys, traffic, and Swagger after (or alongside) migration (xref:11-kuadrant-console.adoc[module 11]).

| *Migration Toolkit*
| Guided discovery from a live 3scale Admin API into Connectivity Link config (xref:12-migration-toolkit.adoc[module 12]).

| *APIShift*
| Same migration core as the Toolkit + AI assist + register into Developer Hub (xref:13-apishift-gateforge.adoc[module 13]).
|===

You will *explore* source and target first (modules 3–6), then *execute* the Golden Path and the UIs. See the xref:index.adoc[Home] strategies table for the full first-impression map.
...
| *APIcast*
| The 3scale API gateway based on NGINX/OpenResty. Handles authentication, rate limiting, and proxying. Docs: https://docs.redhat.com/en/documentation/red_hat_3scale_api_management/[Red Hat 3scale].
...
| *PlanPolicy*
| Defines tiered usage plans (free/basic/pro) with different rate limits via https://github.com/google/cel-spec[CEL] predicates (expression language used in Kuadrant policies).

| *APIProduct*
| Publishes an API to the Kuadrant developer portal (Backstage plugin).

| *Authorino*
| The policy engine that evaluates AuthPolicy rules (JWT, API Key, OPA, pattern matching). Docs: https://docs.kuadrant.io/latest/authorino/docs/getting-started/[Authorino getting started].

| *Limitador*
| The rate-limiting engine that enforces RateLimitPolicy counters via Envoy integration. Docs: https://docs.kuadrant.io/latest/architecture/docs/design/architectural-overview-v1/[Kuadrant architecture] · https://docs.kuadrant.io/latest/kuadrant-operator/doc/user-guides/observability/limitador-metrics/[Limitador metrics].
|===
...
| *API Gateway*
| https://docs.redhat.com/en/documentation/red_hat_3scale_api_management/2.16/html/installing_3scale/installing-threescale[APIcast] (NGINX-based, proprietary config)
...
| PlanPolicy (https://github.com/google/cel-spec[CEL] predicates on identity metadata)
|===
...
Continue with xref:02-architecture.adoc[Architecture Overview].
