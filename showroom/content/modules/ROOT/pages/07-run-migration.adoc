= Golden Path: Developer Hub Software Template

[.lead]
*Estimated time*: ~25 minutes · *Strategy*: Developer Hub Golden Path

== Learning objectives

* Run the *Migrate from 3scale to Connectivity Link* Software Template — the workshop *Golden Path*
* Understand the manifests the scaffolder generates (Gateway, HTTPRoute, policies, APIProduct)
* Confirm Gitea publish + Argo CD Application creation for your user namespace

This is the *self-service GitOps* strategy in the workshop set (alongside xref:11-kuadrant-console.adoc[Kuadrant Console], xref:12-migration-toolkit.adoc[Migration Toolkit], and xref:13-apishift-gateforge.adoc[APIShift]).
...
== The rest of the strategy set (same target)

You just ran the *Golden Path*. The same Connectivity Link resource types can also come from the guided UIs; day-2 ops land in the Console:

[cols="1,2",options="header"]
|===
| Strategy | When to use it

| xref:12-migration-toolkit.adoc[Migration Toolkit]
| Guided discovery against 3scale Admin API; no AI

| xref:13-apishift-gateforge.adoc[APIShift]
| Same migration core + AI models (`litemaas`) + register the component in Developer Hub

| xref:11-kuadrant-console.adoc[Kuadrant Console]
| After policies exist — products, keys, traffic, Swagger
|===

== Next

In the next module you will xref:08-verify-test.adoc[verify the migrated application] and compare it with the 3scale original.
