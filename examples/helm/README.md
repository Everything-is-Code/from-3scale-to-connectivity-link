...
**Migration Toolkit RHCL:** Git-sourced Helm app (`connectivityLink.helmApps` id `migration-toolkit-rhcl`, enabled). Canonical docs/repo: [Everything-is-Code/migration-toolkit-rhcl](https://github.com/Everything-is-Code/migration-toolkit-rhcl) (`helm/migration-toolkit-rhcl`). Namespace `migration-toolkit`, route `migration-toolkit.<domain>`, images `quay.io/maximilianopizarro/migration-toolkit-rhcl-{backend,frontend}:v0.1.0`. ConsoleLink: **Migration Toolkit RHCL** (ApplicationMenu → API Management).
